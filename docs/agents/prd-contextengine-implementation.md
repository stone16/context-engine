...
- 公开 prior art 在 #205 保持 open 时只允许引用四个已准入固定版本的公开仓：Dify、RAGFlow、MaxKB、Onyx；证据索引统一见 `docs/research/2026-08-02-five-public-repositories-evidence.md`。该基线另含 OpenViking 候选包，但本 PRD 不把 OpenViking 引作 authority；候选范围只包括 context filesystem/L0–L2 tiering、session→candidate UX、observable trajectory 与 agent exposure 四类 claim，禁止作为多租户授权/安全证明、Runtime foundation 或任何 copy+patch 来源。
