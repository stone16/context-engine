...
| Deployment/plugin composition | [0101 — Compose deployments from statically registered plugins](0101-compose-deployments-from-statically-registered-plugins.md) | Any future plugin is pinned and statically wired at an accepted port/process; generic manifest and loader design wait for a second real implementation or measured deployment drift | Tenant-uploaded code, hot loading, speculative plugin categories, plugin-owned policy/persistence, implicit fallback, or a plugin-created service boundary |
| Active-carrier completeness | [0108 — Make one registry the active-carrier completeness authority](0108-make-one-registry-the-active-carrier-completeness-authority.md) | `eval/catalogs/active-carriers-v1.json` is the canonical cross-carrier inventory, status, and completeness authority; per-carrier accepted ADRs remain behavioral-authority inputs and `STATUS.md` is a checked human mirror | Activating behavior by registry entry, treating completeness as Security `PASS`, replacing per-carrier ADRs, external/escaping references, status drift, or omitted known deferred carriers |
...
| Agent operation discovery | [0098 — Expose current agent operations without granting authority](0098-expose-current-agent-operations-without-granting-authority.md) | A future short-lived generated manifest intersects contract operations, active carriers, and authenticated-application route policy, and binds the exact AgentVersion without redefining its ceiling | Handwritten route catalogs, discovery as a grant, a second application profile/ceiling, resource enumeration, cached session authority, or activating an operation by listing it |
...
| First structured acquisition workload | [0099 — Select live surfaces as the first structured acquisition workload](0099-select-live-surfaces-as-the-first-structured-acquisition-workload.md) | Bounded request-time reads from the exact private conversation redeemed into TrustedDeliveryContext become the first workload; another target requires a new versioned contract | Snapshotting live results, caller-selected v0 conversation targets, direct surface-to-model delivery, caller-authored viewer/audience facts, partial fallback, or activation before the structured-family contract exists |
...
| Private effect preparation | [0049 — Exact private ActionPlane prepare](0049-prepare-one-exact-private-effect.md) | Trusted TypeScript intent, operation-specific ticket, and function-only digest/RLS persistence revalidate one private effect with atomic idempotency before Sender | Model/wire-derived intent, generic operation tickets, application-role table access, bearer/payload persistence, or claiming prepare as perform |
| Private effect execution | [0050 — Exact private ActionPlane perform](0050-perform-one-exact-private-effect.md) | One durable provider-attempt identity fences Sender, receipt replay, and monotonic applied/rejected reconciliation after exact current-authority validation | Retrying ambiguity under a new identity, cross-operation tickets, Sender before validation, mutable receipts, raw payload/bearer retention, or claiming the deterministic twin as a real provider |
...
| Session-derived Learning | [0100 — Admit session-derived Learning only as governed candidates](0100-admit-session-derived-learning-only-as-governed-candidates.md) | Consented minimized session projections may produce immutable provenance-bound candidates; a separate governed-memory artifact and Runtime-serving ADR is mandatory before any candidate enters release or ContextPackage delivery | Transcript-as-corpus, assistant/tool text as user truth, missing-consent fallback, reusing Revision-bound curation contracts, or extractor-owned active memory/release mutation |
...
| Golden evaluation security | [0080 — Refuse authoritative evaluation without an executor](0080-refuse-authoritative-evaluation-without-an-executor.md) | One executor privately constructs security observations from responses it fetched itself through the recorded `dogfood-loopback-resolve-acquire-v1` seam; file-only runs retain layered metrics but stay explicitly `REFUSED` | Caller-authored counters/events/callbacks/transports, synthetic factory imports outside tests, clean attestation without execution, publication authority for the executor, or quality scores offsetting security |
| Golden corpus survival | [0082 — Recover the golden corpus and refuse stale lineage](0082-recover-the-golden-corpus-and-refuse-stale-lineage.md) | A second durable root holds immutable staged-then-renamed snapshots with per-file and per-snapshot digests; unresolvable expectation lineage refuses the report | Backups inside the repository, a worktree, or the corpus root; partial or unverified snapshots restored silently; stale lineage scored as a retrieval miss; corpus paths in arguments, logs, or refusals |
...
| Rich Markdown compiler-runner | [0079 — Rich Markdown in an owned runner](0079-compile-rich-markdown-in-an-owned-runner.md) | Explicit v3 pure compilation accepts the delimiter-complete closed rich grammar with exact source spans, exact provenance identity, same-Fragment ancestry, a hard token ceiling, and bounded typed runner termination while production activation stays deferred | Silent v1/v2 reinterpretation, position-dependent thematic-break refusal, relabeled v3 provenance, runner-local state, unbounded waits or Fragments, or activating publication without a separate decision |
| Authorized one-hop graph expansion | [0093 — Activate leased rich Markdown and the Revision link graph](0093-activate-leased-rich-markdown-and-revision-link-graph.md) | Lease-selected v3 publication persists immutable content-free Revision edges; one hop starts from authorized main projections, inherits only verified same-Article/current-Revision lineage, re-authorizes every cross-Article candidate, and competes in authorized ranking | Scanner-owned compilation, old-Revision backfill, recursive traversal, link/Fragment ACLs, graph/index authorization, or auto-included neighbours |
...
| File Source progress | [0043 — Separate acquisition and publication progress](0043-separate-file-acquisition-progress-from-publication-progress.md), [0072 — Report File source status with closed refusals](0072-report-file-source-status-with-closed-refusals.md) | Append accepted changes separately from contiguous Runtime-visibility completion and expose content-free operational status through an Organization/Source-scoped Control read | One ambiguous checkpoint, skipped publication gaps, retained source/compiler diagnostics, Runtime authorization from status or watermarks, or false standard ProviderPort capability claims |
...
| Autonomous File dispatch | [0059 — Dispatch scheduled File imports through exact leases](0059-dispatch-scheduled-file-imports-through-exact-leases.md) | A function-only scheduler login atomically claims the oldest current page-scheduled upsert and mints the existing exact first-attempt WorkerLease | Caller tenant/job routing, broad Control credentials, direct scheduler table access, retry/reclaim, delete execution, or a second queue/process |
...
| Candidate discovery capability | [0081 — Seal candidate discovery behind a data-only session](0081-seal-candidate-discovery-behind-a-data-only-session.md) | Runtime executes a narrow prepared request on the retained session; replaceable pre-Kernel code receives only a data-only result session | Passing the projection session, persistence port, connection, locator, projector, or any reachable content capability to a CandidateIndex |
| Loopback dogfood Runtime | [0068 — Activate the loopback dogfood Runtime](0068-activate-loopback-dogfood-runtime.md) | One explicit single-Membership File Acquire carrier applies exact EffectiveScope before ANN LIMIT, then traverses the sealed Kernel with deterministic twin queries | Production/multi-user authentication, remote ingress, group/public/Continue, hybrid/non-File retrieval, or external query embeddings |
| Local MCP ingress | [0103 — Activate one local MCP Acquire translation](0103-activate-one-local-mcp-acquire-translation.md) | One spawn-per-session stdio tool translates the exact closed Acquire and outcome through the existing loopback HTTP carrier | Engine/provider imports, trusted tool fields, Package caching, remote/multi-user MCP, Continue/OpenCitation, generation, effects, or egress-grant handling |
| Local read-only consumers | [0088 — Bind local consumers to fresh evidence-bearing Packages](0088-bind-local-consumers-to-fresh-evidence-bearing-packages.md) | Each question acquires one fresh unexpired loopback Package and renders every Block with its single Evidence ref while the environment-held bearer stays out of invocation and session state | Package reuse, post-expiry use, stripped citation lineage, persisted or argument-carried secrets, or widening the local consumer into BotDelivery |
...
| Local evidence console | [0090 — Admit a co-resident local evidence console](0090-admit-a-co-resident-local-evidence-console.md) | One explicitly authenticated server-rendered UI calls typed HTTP carriers inside the API ingress while frozen OpenAPI v0 stays unchanged; Control jobs additionally consume one separate exact-operation Control call | Anonymous process-identity inheritance, Membership-as-Control, direct UI-to-engine calls, denied rank/score leakage, or feedback publication authority |
| Format-neutral compilation | [0094 — Admit a format-neutral parsed document family](0094-admit-a-format-neutral-parsed-document-family.md) | One ParsedDocument family with nominal per-format locators, a closed structural-kind vocabulary including FIGURE, versioned format profiles, and Revision identity binding the full provenance chain | PDF or DOCX provenance disguised as Markdown byte spans, cross-format locator fallback, silent figure omission, or any third-party copy or PDF model activation inside this decision |
| Runtime rerank bridge | [0095 — Bridge Runtime rerank through one internal pre-rerank Package](0095-bridge-runtime-rerank-through-one-internal-package.md) | One internal undelivered audience-bound pre-rerank Package feeds the single nominal AuthorizedModelInput contract through a mandatory internal EgressGate and one-shot internal model-hop grant, with rerank output an exact Evidence permutation | A second same-name contract with different construction rules, caller-visible internal grants or Packages, rerank scores entering authorization or Package fields, or projection-fed provider requests |
| Release-bound token accounting | [0096 — Bind Package token accounting to a release tokenizer](0096-bind-package-token-accounting-to-release-tokenizer.md) | One ReleaseManifest-pinned tokenizer profile and one resolve-owned cumulative meter (tokens, calls, cost, elapsed) published and digest-bound in the final Package through a reviewed new contract version | Byte-profile or provider-tokenizer fallback, stage-local meters and usage resets, frozen-v0 mutation, retroactive recounting, or mixed tokenizer identities within one resolve |
| Local query embedding activation | [0102 — Activate release-bound local query embedding](0102-activate-release-bound-local-query-embedding.md) | One hash-verified local Qwen profile binds Release and Fragment vectors, refuses every mixed-profile corpus, debits an internal query budget, and activates only through promotion | Dimension-only compatibility, twin/network fallback, unmetered query inference, frozen-v0 cumulative claims, partial re-embed promotion, or non-Learning activation |
...
| Private model egress | [0052 — Gate model generation by one authorized Package](0052-gate-model-generation-by-package.md) | One nominal SDK-Package input and exact one-shot model grant gate the deterministic private ModelGateway, bounded answer, Package-subset citations, and digest-only audit | Arbitrary prompt context, multiple Packages, direct Gateway invocation, invented citations, effect intents, or unaudited answer release |
| Private Bot delivery | [0053 — Compose one private File-backed Bot delivery](0053-compose-one-private-bot-delivery.md) | One independent trusted TypeScript Bot process verifies private twin events, resolves only through the installed SDK, generates from one Package, performs distinct placeholder/final effects, and retains a digest-only receipt audit | Engine imports, caller-authored trust, handwritten Runtime calls, shared effect tickets, answer/bearer retention, hidden ambiguous retry, or a fourth process |
| Bounded live Feishu delivery | [0089 — Bound live Feishu to the existing private Bot composition](0089-bound-live-feishu-to-the-existing-private-bot-composition.md) | Verified asker-private events and exact Sender effects may replace only the provider-facing twins in the existing Bot process after live conformance evidence | Treating event fields or channel grants as trust/write authority, adding a process, activating credentials before evidence, group/public delivery, or conflating delivery with Feishu ingestion |
| Feishu subject and freshness authority | [0092 — Authorize Feishu subjects and bound Mirrored freshness](0092-authorize-feishu-subjects-and-bound-mirrored-freshness.md) | Engine-owned mappings recompute exact Article grants and Runtime expires Mirrored Feishu evidence under one closed five-minute profile | Runner-authored local grants, cross-Article artifact replay, retained grants after observation failure, unbounded mirrors, Weak fallback, or downgrade with retained Feishu state |
...
- [0079 — Compile rich Markdown in an owned runner](0079-compile-rich-markdown-in-an-owned-runner.md)
...
- [0088 — Bind local consumers to fresh evidence-bearing Packages](0088-bind-local-consumers-to-fresh-evidence-bearing-packages.md)
- [0089 — Bound live Feishu to the existing private Bot composition](0089-bound-live-feishu-to-the-existing-private-bot-composition.md)
- [0090 — Admit a co-resident local evidence console](0090-admit-a-co-resident-local-evidence-console.md)
...
- [0098 — Expose current agent operations without granting authority](0098-expose-current-agent-operations-without-granting-authority.md)
- [0099 — Select live surfaces as the first structured acquisition workload](0099-select-live-surfaces-as-the-first-structured-acquisition-workload.md)
- [0100 — Admit session-derived Learning only as governed candidates](0100-admit-session-derived-learning-only-as-governed-candidates.md)
...
- [0106 — Materialize registered models through one operator boundary](0106-materialize-registered-models-through-one-operator-boundary.md)
- [0107 — Bind daily-driver launch to exact schema state](0107-bind-daily-driver-launch-to-exact-schema-state.md)
- [0108 — Make one registry the active-carrier completeness authority](0108-make-one-registry-the-active-carrier-completeness-authority.md)
