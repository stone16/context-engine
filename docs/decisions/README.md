...
| First effective scope | [0024 — Finite target EffectiveScope](0024-model-effective-scope-as-finite-target-intersection.md) | Seven required trusted finite target sets intersect; omitted RequestNarrowing is an explicit identity and supplied narrowing only filters | Stringly permission DSLs, missing-as-unrestricted, Agent-as-grant semantics, or caller-authored trusted scope |
