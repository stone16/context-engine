...
| M1 golden evaluation security | [0080 — Refuse authoritative evaluation without an executor](0080-refuse-authoritative-evaluation-without-an-executor.md) | File-only runs retain layered metrics but remain explicitly `REFUSED` until a real executor privately constructs security observations | Caller-authored counters/events/callbacks, synthetic factory imports outside tests, clean attestation without execution, or quality scores offsetting security |
...
- [0086 — Bind local consumers to fresh evidence-bearing Packages](0086-bind-local-consumers-to-fresh-evidence-bearing-packages.md)
- [0087 — Bound live Feishu to the existing private Bot composition](0087-bound-live-feishu-to-the-existing-private-bot-composition.md)
