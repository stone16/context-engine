...
| Format-neutral compilation | [0091 — Admit a format-neutral parsed document family](0091-admit-a-format-neutral-parsed-document-family.md) | One ParsedDocument family with nominal per-format locators, a closed structural-kind vocabulary including FIGURE, versioned format profiles, and Revision identity binding the full provenance chain | PDF or DOCX provenance disguised as Markdown byte spans, cross-format locator fallback, silent figure omission, or any third-party copy or PDF model activation inside this decision |
| Runtime rerank bridge | [0092 — Bridge Runtime rerank through one internal pre-rerank Package](0092-bridge-runtime-rerank-through-one-internal-package.md) | One internal undelivered audience-bound pre-rerank Package feeds the single nominal AuthorizedModelInput contract through a mandatory internal EgressGate and one-shot internal model-hop grant, with rerank output an exact Evidence permutation | A second same-name contract with different construction rules, caller-visible internal grants or Packages, rerank scores entering authorization or Package fields, or projection-fed provider requests |
| Release-bound token accounting | [0093 — Bind Package token accounting to a release tokenizer](0093-bind-package-token-accounting-to-release-tokenizer.md) | One ReleaseManifest-pinned tokenizer profile and one resolve-owned cumulative meter (tokens, calls, cost, elapsed) published and digest-bound in the final Package through a reviewed new contract version | Byte-profile or provider-tokenizer fallback, stage-local meters and usage resets, frozen-v0 mutation, retroactive recounting, or mixed tokenizer identities within one resolve |
