...
Organization、job、ServiceActor/workload、worker audience、过期时间与 nonce，并在
non-owner FORCE RLS 下只允许一次原子完成；真实 ingestion、outbox 与 publication
job 仍为 `NOT_ACTIVE`。整体计划见 [PLAN.md](./PLAN.md)。
...
事务时间和 server-owned bounded TTL 签出租约；worker 应用 seam 必须先以自身配置的
ServiceActor/workload/audience 与时钟验证签名、Organization、job、固定 operation 和时效，
再打开数据库事务。worker 无直接 job UPDATE 权限；专用 non-login definer function 在
FORCE RLS 下以数据库当前时间、key version、nonce digest、issued-at/expiry 做一次条件
更新。有效 lease 的 effect count
...
worker loop，也不把完整 `ACCEPT-008` 标为通过。
