...
const snapshotPath = resolve(repositoryRoot, "openapi/v1/openapi.json");
const snapshotDigestPath = resolve(repositoryRoot, "openapi/v1/openapi.sha256");
const generatedPath = resolve(sdkRoot, "src/generated");
const generatedDigestPath = resolve(sdkRoot, "src/generated.sha256");
const contractDigestPath = resolve(sdkRoot, "contract/openapi-v1.sha256");
...
if (observedSnapshotDigest !== expectedSnapshotDigest) {
  throw new Error("OpenAPI v1 snapshot does not match its accepted checksum");
}
...
try {
  ...
  if (checkOnly) {
    ...
    if (packagedContractDigest !== expectedSnapshotDigest) {
      throw new Error("packaged OpenAPI v1 provenance checksum drifted");
    }
