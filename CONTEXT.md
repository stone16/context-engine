...
| `ContextSource` | Persistent logical source registration with terminal active/disabled lifecycle | One Organization | Current active lifecycle is required, never sufficient alone |
...
- **Lifecycle:** stable while configuration/capability snapshots are represented
  by SourceVersions; the first committed offboarding terminally changes the
  Source from active to disabled without deleting either identity or content.
- **Invariant:** owns ContextResources and declares source policy/capabilities;
  callers and Agents cannot choose a weaker source mode. A disabled Source
  contributes no Runtime or Worker authority even while physical artifacts
  remain.
...
- **Owner/scope:** one Organization, authenticated identity chain, declared
  source-read audience, optional exact source binding, purpose, revocation
...
- **Invariant:** valid only for its exact read audience/restrictions; Provider
  projection evidence still requires Kernel validation. A source-bound ticket
  can be minted and redeemed only while that exact Source is active.
...
### `File source cleanup intent`

The immutable obligation to remove retained artifacts after one File
ContextSource is already synchronously disabled. 中文：File source cleanup intent
在 ContextSource disable 与 Organization Policy Epoch 推进的同一事务中创建；它
记录后续 source-wide 物理清理责任，但不决定 Runtime 可见性。

- **Owner/scope:** one Organization, disabled ContextSource, and the exact
  immutable active SourceVersion recorded at offboarding.
- **Lifecycle:** created once in `pending`; Issue #30 does not activate cleanup,
  completion, retention expiry, or source re-enable.
- **Invariant:** source disable, Policy Epoch advancement, outstanding-job
  cancellation, and intent creation commit atomically. Retained Resource,
  Revision, Fragment, candidate, and progress rows remain non-authoritative.
- **Do not confuse with:** per-Resource File cleanup intent, physical cleanup,
  acquisition checkpoint, publish watermark, or credential revocation.
