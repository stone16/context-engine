...
def _contract_from_environment(
    ...
    if environment["CONTEXT_ENGINE_ACTION_ROLE"] != ACTION_ROLE:
        raise ValueError("database role provisioning has an invalid action role")
    if environment["CONTEXT_ENGINE_SCHEDULER_ROLE"] != SCHEDULER_ROLE:
    ...
    return RoleProvisioningContract(
        ...
        action_role=environment["CONTEXT_ENGINE_ACTION_ROLE"],
        action_password=environment["CONTEXT_ENGINE_ACTION_PASSWORD"],
        scheduler_role=environment["CONTEXT_ENGINE_SCHEDULER_ROLE"],
...
def provision_security_roles(
    ...
    """Create or reconcile post-init roles and bootstrap-owned extensions."""
    ...
    connection.execute(
        sql.SQL(
            ...
            sql.Identifier(contract.scheduler_role),
            sql.Literal(contract.scheduler_password),
        )
    )
    connection.execute(
        sql.SQL(
    ...
    connection.execute(
        sql.SQL(
            ...
            sql.Identifier(contract.action_role),
            sql.Literal(contract.action_password),
        )
    )
    connection.execute(
        sql.SQL(
    ...
    connection.execute(
        sql.SQL(
            ...
            sql.Identifier(contract.security_operator_role),
            sql.Literal(contract.security_operator_password),
        )
    )
    connection.execute(
        sql.SQL(
    ...
    connection.execute(
        sql.SQL(
            "ALTER ROLE {} WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE "
            "NOINHERIT NOREPLICATION NOBYPASSRLS"
        ).format(sql.Identifier(contract.action_prepare_definer_role))
    )
    connection.execute(
        sql.SQL(
            "ALTER ROLE {} WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE "
            "NOINHERIT NOREPLICATION NOBYPASSRLS"
        ).format(sql.Identifier(contract.action_execute_definer_role))
    )
    connection.execute(
        sql.SQL(
            "ALTER ROLE {} WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE "
    ...
    connection.execute(
        sql.SQL(
            "ALTER ROLE {} WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE "
            "NOINHERIT NOREPLICATION NOBYPASSRLS"
        ).format(sql.Identifier(contract.file_dispatch_definer_role))
    )
    connection.execute(
        sql.SQL(
            "ALTER ROLE {} WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE "
            "NOINHERIT NOREPLICATION NOBYPASSRLS"
        ).format(sql.Identifier(contract.context_run_reader_definer_role))
    )
    connection.execute(
        sql.SQL(
            "ALTER ROLE {} WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE "
            "NOINHERIT NOREPLICATION NOBYPASSRLS"
        ).format(sql.Identifier(contract.release_definer_role))
    )
    ...
    connection.execute(
        sql.SQL("GRANT {} TO {} WITH ADMIN FALSE, INHERIT FALSE, SET TRUE").format(
            sql.Identifier(contract.action_prepare_definer_role),
            sql.Identifier(contract.migrator_role),
        )
    )
    connection.execute(
        sql.SQL("GRANT {} TO {} WITH ADMIN FALSE, INHERIT FALSE, SET TRUE").format(
            sql.Identifier(contract.action_execute_definer_role),
            sql.Identifier(contract.migrator_role),
        )
    )
    connection.execute(
        sql.SQL("GRANT {} TO {} WITH ADMIN FALSE, INHERIT FALSE, SET TRUE").format(
            sql.Identifier(contract.context_run_reader_definer_role),
            sql.Identifier(contract.migrator_role),
        )
    )
    connection.execute(
        sql.SQL("GRANT {} TO {} WITH ADMIN FALSE, INHERIT FALSE, SET TRUE").format(
            sql.Identifier(contract.worker_lease_definer_role),
            sql.Identifier(contract.migrator_role),
        )
    )
    connection.execute(
        sql.SQL("GRANT {} TO {} WITH ADMIN FALSE, INHERIT FALSE, SET TRUE").format(
            sql.Identifier(contract.file_dispatch_definer_role),
            sql.Identifier(contract.migrator_role),
        )
    )
    connection.execute(
