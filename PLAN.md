...
- **下游**:统一的 `ContextRuntime.resolve(AuthenticatedInvocation, TrustedDeliveryContext, Acquire | Continue | OpenCitation)` 首先经 HTTP 交付 ContextPackage;TypeScript SDK 是由 OpenAPI 生成的 HTTP client,不是服务端 transport。ADR-0102 只激活维护者本地、spawn-per-session stdio MCP Acquire translator;其余 MCP carrier 保持 NOT_ACTIVE。其上的 **BotDelivery** 深模块编排受信身份/audience 证据、生成与 IM egress,但不计算授权 scope。
- **差异化**:
